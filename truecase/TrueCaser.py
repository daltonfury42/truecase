...
class TrueCaser(object):
    ...
    def get_score(self, prev_token, possible_token, next_token):
        ...
        denominator = 0
        for alternative_token in self.word_casing_lookup[
                possible_token.lower()]:
            denominator += self.uni_dist[alternative_token] + pseudo_count
        ...
        if next_token is not None:
            ...
            nominator = (
                ...
                pseudo_count)
            denominator = 0
            for altertantive_token in self.word_casing_lookup[
                    possible_token.lower()]:
                denominator += (
                    self.forward_bi_dist[alternative_token + "_" + next_token] +
                    pseudo_count)
        ...
        if prev_token is not None and next_token is not None:
            ...
            for alternative_token in self.word_casing_lookup[
                    possible_token.lower()]:
                denominator += (
                    self.trigram_dist[prev_token + "_" + alternative_token +
                                      "_" + next_token] + pseudo_count)
    ...
    def _fix_upper_contractions(self, sentence):
        ...
        for idx, char in enumerate(fix_sentence):
            if char.isupper() and previous_char == "'":
